...
the SYCL-TLA kernels provided by `sgl-kernel-xpu`. Q4_K and Q8_0 values are
prepared once while their block metadata stays compact; the serving path does
not depend on the llm-scaler ESIMD extension.
...
- Q4_K linear and MoE gate/up weights use the direct compact-metadata kernel;
  Q8_0 linear and MoE down weights use the direct signed-INT8 kernel. Other
