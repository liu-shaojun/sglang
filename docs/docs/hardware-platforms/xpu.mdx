...
Intel XPU supports text-generation checkpoints in the GGUF Q4_K family through
the SYCL-TLA kernels provided by `sgl-kernel-xpu`. Q4_K and Q8_0 values remain
compact. IQ3_S, IQ4_NL, and IQ4_XS are prepared as signed codebook values, and
Q5_1 and Q5_K as affine INT8 codes, all with 32-value group scales; Q3_K and
Q6_K use signed INT8 with 16-value group scales. These representations use less
memory than BF16 expansion. The serving path does not depend on the llm-scaler
ESIMD extension.

Place the Hugging Face sidecar files next to the GGUF file. At minimum this
includes `config.json`, the tokenizer files, and
`model.safetensors.index.json` (or the corresponding safetensors shards). This
uses the same sidecar discovery as the existing SGLang GGUF loader; no
XPU-specific environment variable is required.
...
Launch the server by passing the GGUF file directly:

```bash
sglang serve \
    --model-path /models/qwen-gguf/model-Q4_K_M.gguf \
    --device xpu \
    --tp 2 \
...
`--disable-radix-cache` is currently required for the hybrid GDN Qwen models:
the Intel XPU attention backend uses a paged KV cache, while the upstream
no-buffer Mamba radix-cache strategy requires page size 1. It can be omitted
for the Gemma4 checkpoints. The custom all-reduce and overlap-schedule flags
match the current recommended XPU serving configuration.
...
- The optimized packed path targets Xe2/BMG Intel GPUs.
- Q4_K linear and MoE gate/up weights use the direct compact-metadata kernel;
  IQ3_S, IQ4_NL, IQ4_XS, Q3_K, Q6_K, and Q8_0 linear and MoE down weights use
  signed-INT8 kernels, while Q5_1 and Q5_K use an affine INT8 kernel. Other
  quantized projections are dequantized once during loading.
- CUDA, ROCm, and other accelerator backends are outside this XPU validation
  scope.
