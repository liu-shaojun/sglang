...
compact. Q6_K is prepared as signed INT8 with its 16-value group scales, which
uses less memory than BF16 expansion. The serving path does not depend on the
llm-scaler ESIMD extension.
