...
compact. IQ4_XS is prepared as signed codebook values, and Q5_K as affine INT8
codes, both with 32-value group scales; Q6_K uses signed INT8 with 16-value
group scales. These representations use less memory than BF16 expansion. The
serving path does not depend on the llm-scaler ESIMD extension.
...
- Q4_K linear and MoE gate/up weights use the direct compact-metadata kernel;
  IQ4_XS, Q6_K, and Q8_0 linear and MoE down weights use signed-INT8 kernels,
  while Q5_K uses an affine INT8 kernel. Other quantized projections are
  dequantized once during loading.
