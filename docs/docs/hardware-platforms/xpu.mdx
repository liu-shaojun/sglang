...
Q5_K as affine INT8 codes, all with 32-value group scales; Q3_K and Q6_K use signed
INT8 with 16-value group scales. These representations use less memory than
BF16 expansion. The serving path does not depend on the llm-scaler ESIMD
extension.
...
- Q4_K linear and MoE gate/up weights use the direct compact-metadata kernel;
  ...
  signed-INT8 kernels, while Q5_K uses an affine INT8 kernel. Other quantized
  projections are dequantized once during loading.
