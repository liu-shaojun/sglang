...
the SYCL-TLA kernels provided by `sgl-kernel-xpu`. GGUF Q4_K values are repacked
once while their 16-byte super-block metadata stays compact; the serving path
does not depend on the llm-scaler ESIMD extension.
...
- Q4_K linear and MoE gate/up weights use the direct compact-metadata kernel.
  Other K-quant MoE down projections are dequantized once to BF16 during loading.
