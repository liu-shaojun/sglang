"""CPU dispatch coverage for the XPU GGUF IQ4_XS path."""
...
def test_xpu_gguf_iq4_xs_moe_loader_keeps_grouped_down_projection(monkeypatch):
    layer = torch.nn.Module()
    layer.w13_qweight = torch.empty((2, 16, 144), dtype=torch.uint8)
    layer.w2_qweight = torch.empty((2, 8, 136), dtype=torch.uint8)
    layer.w13_qweight_type = SimpleNamespace(weight_type=WeightType.Q4_K.value)
    layer.w2_qweight_type = SimpleNamespace(weight_type=WeightType.IQ4_XS.value)
    ...
    def fake_prepare_iq4_xs(raw):
        assert raw.shape == (16, 136)
        return (
    ...
    monkeypatch.setattr(
        gguf_module, "_xpu_prepare_iq4_xs", fake_prepare_iq4_xs
    )
    ...
    assert layer.w2_xpu_kind == "iq4_xs"
    assert not hasattr(layer, "w13_qweight")
    assert not hasattr(layer, "w2_qweight")


def test_xpu_gguf_iq4_xs_linear_dispatch_reuses_group32_gemm(monkeypatch):
    layer = torch.nn.Module()
    prefix = "_gguf_xpu_0"
    ...
    layer._gguf_xpu_representations = [("iq4_xs", prefix, None, None)]
