"""CPU dispatch coverage for the XPU GGUF IQ3_S path."""
...
def test_xpu_gguf_iq3_s_moe_loader_keeps_grouped_down_projection(monkeypatch):
    ...
    layer.w2_qweight_type = SimpleNamespace(weight_type=WeightType.IQ3_S.value)
    ...
    monkeypatch.setattr(gguf_module, "_xpu_prepare_iq3_s", fake_prepare_iq3_s)
    ...
    assert layer.w2_xpu_kind == "iq3_s"
    assert not hasattr(layer, "w13_qweight")
    assert not hasattr(layer, "w2_qweight")


def test_xpu_gguf_iq3_s_linear_dispatch_reuses_group32_gemm(monkeypatch):
    layer = torch.nn.Module()
    prefix = "_gguf_xpu_0"
    ...
    layer._gguf_xpu_representations = [("iq3_s", prefix, None, None)]
