...
class GGUFMoEXPUMethod(GGUFMoEMethod):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module):
        ...
        packed, metadata = _xpu_prepare_q4_k(
            w13.detach()
        ...
        layer.register_buffer(
            "w13_xpu_qweight",
            packed.reshape(num_experts, output_features, packed.shape[-1]),
            persistent=False,
        )
        layer.register_buffer(
            "w13_xpu_metadata",
            metadata.reshape(num_experts, output_features, metadata.shape[-1]),
            persistent=False,
        )
        ...
        if w2_type in (
            ...
            minimums = None
            if w2_type == WeightType.IQ3_S:
                weights, scales = _xpu_prepare_iq3_s(raw_w2)
                layer.w2_xpu_kind = "iq3_s"
            elif w2_type == WeightType.IQ4_NL:
                weights, scales = _xpu_prepare_iq4_nl(raw_w2)
                layer.w2_xpu_kind = "iq4_nl"
            elif w2_type == WeightType.IQ4_XS:
                ...
                weights, scales = _xpu_prepare_q6_k(raw_w2)
                layer.w2_xpu_kind = "q6_k"
            else:
                weights, scales = _xpu_prepare_q8_0(raw_w2)
            ...
            if minimums is not None:
                layer.register_buffer(
                    "w2_xpu_minimums",
                    minimums.reshape(
                        num_experts, output_features, minimums.shape[-1]
                    ),
                    persistent=False,
                )
        else:
    ...
    def apply(
        ...
        torch.ops.sgl_kernel.gguf_q4_k_grouped_mm(
            gate_up,
            routed_x,
            layer.w13_xpu_qweight,
            layer.w13_xpu_metadata,
            expert_rows,
            num_experts,
        )
        ...
        if layer.w2_xpu_kind in (
            ...
            elif layer.w2_xpu_kind in ("q3_k", "q6_k"):
                torch.ops.sgl_kernel.gguf_q6_k_grouped_mm(
                    routed_output,
                    activated,
                    layer.w2_xpu_qweight,
                    layer.w2_xpu_scales,
...
def _xpu_prepare_q4_k(
    raw_weight: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Prepare Q4_K values plus their compact 16-byte super-block metadata."""
    ...
    if raw_weight.dtype != torch.uint8 or raw_weight.ndim != 2:
        raise ValueError("Q4_K preparation expects a rank-2 uint8 tensor")
    ...
    # the current worker device explicitly before dispatching it.
    device = torch.device("xpu", torch.xpu.current_device())
    raw_weight = raw_weight.to(device=device).contiguous()
    rows = raw_weight.shape[0]
    blocks = raw_weight.shape[1] // Q4_K_BLOCK_BYTES
    packed = torch.empty(
        (rows, blocks * Q4_K_SUPER_BLOCK_SIZE // 2),
        dtype=torch.uint8,
        device=device,
    )
...
def _xpu_prepare_iq3_s(
    raw_weight: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Expand IQ3_S codebook values with one FP16 scale per 32 values."""
    block_bytes = 110
    values_per_block = 256
    groups_per_block = 8
    if raw_weight.dtype != torch.uint8 or raw_weight.ndim != 2:
        raise ValueError("IQ3_S preparation expects a rank-2 uint8 tensor")
    if raw_weight.shape[0] == 0:
        raise ValueError("IQ3_S preparation expects at least one row")
    if raw_weight.shape[1] == 0 or raw_weight.shape[1] % block_bytes:
        raise ValueError(f"IQ3_S byte width must be a multiple of {block_bytes}")
    device = torch.device("xpu", torch.xpu.current_device())
    raw_weight = raw_weight.to(device=device).contiguous()
    rows = raw_weight.shape[0]
    blocks = raw_weight.shape[1] // block_bytes
    ...
    torch.ops.sgl_kernel.gguf_iq3_s_prepare(raw_weight, weights, scales)
    return weights, scales


def _xpu_prepare_iq4_nl(
    raw_weight: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Expand IQ4_NL codebook values with one FP16 scale per 32 values."""
    block_bytes = 18
    values_per_block = 32
    if raw_weight.dtype != torch.uint8 or raw_weight.ndim != 2:
        raise ValueError("IQ4_NL preparation expects a rank-2 uint8 tensor")
    if raw_weight.shape[0] == 0:
        raise ValueError("IQ4_NL preparation expects at least one row")
    if raw_weight.shape[1] == 0 or raw_weight.shape[1] % block_bytes:
        raise ValueError(f"IQ4_NL byte width must be a multiple of {block_bytes}")
    device = torch.device("xpu", torch.xpu.current_device())
    raw_weight = raw_weight.to(device=device).contiguous()
    rows = raw_weight.shape[0]
    blocks = raw_weight.shape[1] // block_bytes
    ...
    scales = torch.empty((rows, blocks), dtype=torch.float16, device=device)
    torch.ops.sgl_kernel.gguf_iq4_nl_prepare(raw_weight, weights, scales)
    return weights, scales
...
def _xpu_prepare_q6_k(
    ...
    """Prepare Q6_K as signed INT8 values with one FP16 scale per 16 values."""
    block_bytes = 210
    values_per_block = 256
    groups_per_block = 16
    if raw_weight.dtype != torch.uint8 or raw_weight.ndim != 2:
        raise ValueError("Q6_K preparation expects a rank-2 uint8 tensor")
    if raw_weight.shape[0] == 0:
        raise ValueError("Q6_K preparation expects at least one row")
    if raw_weight.shape[1] == 0 or raw_weight.shape[1] % block_bytes:
        raise ValueError(f"Q6_K byte width must be a multiple of {block_bytes}")
    device = torch.device("xpu", torch.xpu.current_device())
    raw_weight = raw_weight.to(device=device).contiguous()
    rows = raw_weight.shape[0]
    blocks = raw_weight.shape[1] // block_bytes
    ...
    torch.ops.sgl_kernel.gguf_q6_k_prepare(raw_weight, weights, scales)
    return weights, scales


def _xpu_prepare_q3_k(
    raw_weight: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Prepare Q3_K as signed INT8 values with one FP16 scale per 16 values."""
    block_bytes = 110
    values_per_block = 256
    groups_per_block = 16
    if raw_weight.dtype != torch.uint8 or raw_weight.ndim != 2:
        raise ValueError("Q3_K preparation expects a rank-2 uint8 tensor")
    if raw_weight.shape[0] == 0:
        raise ValueError("Q3_K preparation expects at least one row")
    if raw_weight.shape[1] == 0 or raw_weight.shape[1] % block_bytes:
        raise ValueError(f"Q3_K byte width must be a multiple of {block_bytes}")
    device = torch.device("xpu", torch.xpu.current_device())
    raw_weight = raw_weight.to(device=device).contiguous()
    rows = raw_weight.shape[0]
    blocks = raw_weight.shape[1] // block_bytes
    ...
    torch.ops.sgl_kernel.gguf_q3_k_prepare(raw_weight, weights, scales)
    return weights, scales


def _xpu_prepare_q5_k(
    raw_weight: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Prepare Q5_K INT8 codes with one FP16 scale/minimum per 32 values."""
    block_bytes = 176
    values_per_block = 256
    groups_per_block = 8
    if raw_weight.dtype != torch.uint8 or raw_weight.ndim != 2:
        raise ValueError("Q5_K preparation expects a rank-2 uint8 tensor")
    if raw_weight.shape[0] == 0:
        raise ValueError("Q5_K preparation expects at least one row")
    if raw_weight.shape[1] == 0 or raw_weight.shape[1] % block_bytes:
        raise ValueError(f"Q5_K byte width must be a multiple of {block_bytes}")
    device = torch.device("xpu", torch.xpu.current_device())
    raw_weight = raw_weight.to(device=device).contiguous()
    rows = raw_weight.shape[0]
    blocks = raw_weight.shape[1] // block_bytes
    ...
    metadata_shape = (rows, blocks * groups_per_block)
    scales = torch.empty(metadata_shape, dtype=torch.float16, device=device)
    minimums = torch.empty_like(scales)
    torch.ops.sgl_kernel.gguf_q5_k_prepare(
...
def _xpu_permute_grouped_k(
    ...
    scales: torch.Tensor,
    col_perm: tuple[int, int, int],
    group_size: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Apply the GDN ratio/head K permutation without crossing scale groups."""
    ratio, num_key_heads, value_head_dim = col_perm
    expected = ratio * num_key_heads * value_head_dim
    if weights.shape[1] != expected:
        raise ValueError(
            "GGUF GDN column permutation does not match weight: "
            f"K={weights.shape[1]}, expected={expected}"
        )
    if value_head_dim % group_size:
        raise ValueError(
            f"GGUF GDN value head dim {value_head_dim} must be divisible by "
            f"quantization group size {group_size}"
        )
    rows = weights.shape[0]
    weights = (
        weights.reshape(rows, ratio, num_key_heads, value_head_dim)
        .transpose(1, 2)
...
def _xpu_permute_grouped_k_affine(
    weights: torch.Tensor,
    scales: torch.Tensor,
    ...
    """Apply the same GDN group permutation to Q5_K scale and minimum."""
    weights, scales = _xpu_permute_grouped_k(
        weights, scales, col_perm, group_size
    )
    ratio, num_key_heads, value_head_dim = col_perm
    rows = minimums.shape[0]
    minimums = (
        minimums.reshape(
            rows,
            ratio,
...
class GGUFLinearXPUMethod(GGUFLinearMethod):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module):
        ...
        for index, (shard_id, raw_weight, qweight_type) in enumerate(shards):
            ...
            if qweight_type == WeightType.Q4_K and gdn_col_perm is None:
                ...
                minimums = None
                if qweight_type == WeightType.IQ3_S:
                    kind = "iq3_s"
                    group_size = 32
                    weights, scales = _xpu_prepare_iq3_s(raw_weight)
                elif qweight_type == WeightType.IQ4_NL:
                    kind = "iq4_nl"
                    group_size = 32
                    weights, scales = _xpu_prepare_iq4_nl(raw_weight)
                elif qweight_type == WeightType.IQ4_XS:
                    kind = "iq4_xs"
                    group_size = 32
                    weights, scales = _xpu_prepare_iq4_xs(raw_weight)
                elif qweight_type == WeightType.Q3_K:
                    kind = "q3_k"
                    group_size = 16
                ...
                elif qweight_type == WeightType.Q6_K:
                    kind = "q6_k"
                    group_size = 16
                    weights, scales = _xpu_prepare_q6_k(raw_weight)
                else:
                    kind = "q8_0"
                    group_size = 32
                    weights, scales = _xpu_prepare_q8_0(raw_weight)
                if gdn_col_perm is not None and minimums is not None:
                    weights, scales, minimums = _xpu_permute_grouped_k_affine(
                        weights, scales, minimums, gdn_col_perm, group_size
                    )
                elif gdn_col_perm is not None:
                    weights, scales = _xpu_permute_grouped_k(
                        weights, scales, gdn_col_perm, group_size
                    )
                layer.register_buffer(
                ...
                if minimums is not None:
                    layer.register_buffer(
                        f"{prefix}_minimums",
                        minimums.unsqueeze(0),
                        persistent=False,
                    )
                representations.append((kind, prefix, shard_id, None))
            else:
    ...
    def _apply_q4_k(
        ...
        metadata = getattr(layer, f"{prefix}_metadata")
        if x.dtype not in (torch.bfloat16, torch.float16):
            raise TypeError(f"GGUF XPU Q4_K expects BF16/FP16, got {x.dtype}")
    ...
    def _apply_q6_k(
        ...
        scales = getattr(layer, f"{prefix}_scales")
        if x.dtype not in (torch.bfloat16, torch.float16):
            raise TypeError(f"GGUF XPU Q6_K expects BF16/FP16, got {x.dtype}")
        output_features = weight.shape[1]
        x_2d = x.reshape(-1, x.shape[-1])
        output = torch.empty(
            (x_2d.shape[0], output_features), dtype=x.dtype, device=x.device
    ...
    def _apply_q5_k(
        ...
        weight = getattr(layer, f"{prefix}_weight")
        scales = getattr(layer, f"{prefix}_scales")
        minimums = getattr(layer, f"{prefix}_minimums")
        if x.dtype not in (torch.bfloat16, torch.float16):
            raise TypeError(f"GGUF XPU Q5_K expects BF16/FP16, got {x.dtype}")
        output_features = weight.shape[1]
        x_2d = x.reshape(-1, x.shape[-1])
        output = torch.empty(
            (x_2d.shape[0], output_features), dtype=x.dtype, device=x.device
    ...
    def apply(
        ...
        outputs = []
        for kind, prefix, _shard_id, _kernel_arg in layer._gguf_xpu_representations:
            weight = getattr(layer, f"{prefix}_weight")
            if kind == "q4_k":
